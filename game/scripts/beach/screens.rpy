...
screen beach_island:
    ...
    imagebutton:
        ...
        if M_aqua.get_state() == S_aqua_treasure_search:
            ...
            hover "objects/object_statue_02b.png"

        else:
            pos (311,124)
            idle "objects/object_statue_03.png"
            hover "objects/object_statue_03b.png"
...
screen treasure_lock:
    ...
    imagemap:
        ground "secret/location_beach_lock.png"
        ...
        if treasure_key in inventory.items:
            if lock01 == 1 and lock02 == 5 and lock03 == 1 and lock04 == 3:
                imagebutton:
                    focus_mask True
                    if not open_lock:
                        pos (460,125)
                        idle "objects/object_keyhole_01.png"
                        hover "objects/object_keyhole_01b.png"
                    else:
                        pos (460,74)
                        idle "objects/object_keyhole_02.png"
                    action SetScreenVariable("open_lock", True), Hide("treasure_lock"), With(fade), Jump("treasure_unlocked")
        ...
        if lock01 > 0:
            imagebutton:
                ...
                hover "buttons/lock_icon_" + str(lock01).zfill(2) + "b.png"
                action If(lock01 < 10, SetScreenVariable("lock01", lock01 + 1), SetScreenVariable("lock01", 0))
        else:
            hotspot (110,407,188,186) background "secret/location_beach_lock.jpg" action SetScreenVariable("lock01", 1)
        ...
        if lock02 > 0:
            imagebutton:
                ...
                hover "buttons/lock_icon_" + str(lock02).zfill(2) + "b.png"
                action If(lock02 < 10, SetScreenVariable("lock02", lock02 + 1), SetScreenVariable("lock02", 0))
        else:
            hotspot (317,407,188,186) background "secret/location_beach_lock.jpg" action SetScreenVariable("lock02", 1)
        ...
        if lock03 > 0:
            imagebutton:
                ...
                hover "buttons/lock_icon_" + str(lock03).zfill(2) + "b.png"
                action If(lock03 < 10, SetScreenVariable("lock03", lock03 + 1), SetScreenVariable("lock03", 0))
        else:
            hotspot (522,407,188,186) background "secret/location_beach_lock.jpg" action SetScreenVariable("lock03", 1)
        ...
        if lock04 > 0:
            imagebutton:
                ...
                hover "buttons/lock_icon_" + str(lock04).zfill(2) + "b.png"
                action If(lock04 < 10, SetScreenVariable("lock04", lock04 + 1), SetScreenVariable("lock04", 0))
        else:
            hotspot (728,407,188,186) background "secret/location_beach_lock.jpg" action SetScreenVariable("lock04", 1)
...
screen treasure_chest:
    imagebutton:
        ...
        hover "objects/object_compass_01b.png"
        action Return()