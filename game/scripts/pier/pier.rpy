...
label terry_button_dialogue:
    ...
    if M_terry.get_state() == S_terry_start:
        ...
        Terry "Oh shit!"
        show sara 1 zorder 2 at Position(xpos=.68,ypos=1.0)
        show tstand 1 zorder 1 at Position(xpos=.99,ypos=1.0)
        pause
        show sara 3
        pause
        show sara 5
        ...
        sara "Terry!"
        show tstand 2 at Position(xpos=.9,ypos=1.0)
        show sara 4 at Position(xpos=.8325,ypos=1.0) with dissolve
        Terry "Oh ho, sorry love... Just took me by surprise is all!"
        Terry "That's him!"
...
screen bait_menu:
    ...
    if special_lure in inventory.items:
        imagebutton:
            ...
            action SetVariable("bait", 4), Return()
            xpos 150
            ypos 150
...
label fishing_minigame:
    ...
    if M_terry.get_state() == S_terry_retire and tigger not in inventory.items:
        $ fish4 = At(ImageReference("fish4"), move_fish4)
        show expression fish4

    $ hook = At(ImageReference("hook"), move_hook)
    $ hook_xpos = 500
    $ hook_ypos = 7

    call screen try_fish(hook, fish1, fish2, fish3)

screen fishing:
    add "backgrounds/menu_ground.png"

screen try_fish(hook, fish1, fish2, fish3):
    if fishing_first:
        if renpy.variant("pc"):
    ...
    if not lower_bait:
        if renpy.variant("pc"):
            if bait == 1:
                ...
                add "buttons/fishing_bait04.png" pos hook_xpos,hook_ypos
            ...
            if bait == 1:
                ...
                imagebutton idle "buttons/fishing_bait04.png" action [SetVariable("lower_bait", True)] pos hook_xpos, hook_ypos
        ...
        if bait == 1:
            ...
            add "buttons/fishing_bait04b.png" pos hook_xpos,hook_ypos

    key "K_RETURN" action SetVariable("lower_bait", True)

    if lower_bait:
        timer 0.01 repeat True action If(time_count > 0, true=[SetVariable("time_count", time_count - 0.01), If(hook_ypos < 805, SetVariable("hook_ypos", hook_ypos +2), [If(M_aqua.get_state() == S_aqua_fishing and bait == 4 and fish5_caught, SetVariable("fish_caught", "aqua"), NullAction()), Jump("fish_caught")])], false=NullAction())

    if fish1_flip and hook_ypos > 330 and hook_ypos < 370 and fish1.xpos+170 > hook_xpos-30 and fish1.xpos+170 < hook_xpos+50 or not fish1_flip and hook_ypos > 330 and hook_ypos < 370 and fish1.xpos > hook_xpos-50 and fish1.xpos < hook_xpos+30:
        if bait == 2 or bait == 4:
            $ fish1_caught = True
    if not fish2_flip  and hook_ypos > 530 and hook_ypos < 570 and fish2.xpos+170 > hook_xpos-30 and fish2.xpos+170 < hook_xpos+50 or fish2_flip and hook_ypos > 530 and hook_ypos < 570 and fish2.xpos > hook_xpos-50 and fish2.xpos < hook_xpos+30:
        if bait == 1 or bait == 4:
            $ fish2_caught = True
    if fish3_flip and hook_ypos > 430 and hook_ypos < 470 and fish3.xpos+170 > hook_xpos-30 and fish3.xpos+170 < hook_xpos+50 or not fish3_flip and hook_ypos > 430 and hook_ypos < 470 and fish3.xpos > hook_xpos-50 and fish3.xpos < hook_xpos+30:
        if bait == 3 or bait == 4:
            $ fish3_caught = True
    if M_terry.get_state() == S_terry_retire and tigger not in inventory.items:
        if not fish4_flip and hook_ypos > 605 and hook_ypos < 645 and fish4.xpos+170 > hook_xpos-30 and fish4.xpos+170 < hook_xpos+50 or fish4_flip and hook_ypos > 605 and hook_ypos < 645 and fish4.xpos > hook_xpos-50 and fish4.xpos < hook_xpos+30:
            if bait == 4:
                $ fish4_caught = True
    if hook_ypos > 770 and hook_xpos > 160 and hook_xpos < 350:
        if bait == 4 and M_aqua.get_state() == S_aqua_fishing:
            $ fish5_caught = True

            imagebutton:
                idle "buttons/fishing_fish05.png"
                action NullAction()
                pos 248,661
    ...
    if fish1_caught:
        ...
        $ lower_bait = False
        add "backgrounds/minigame06a.jpg"
        add im.Flip("buttons/fishing_fish04a.png", horizontal = True) pos hook_xpos-170,hook_ypos
        add "buttons/fishing_done.png" pos hook_xpos-70,hook_ypos-90
        add "buttons/fishing_exclamation.png" pos 800,90
        imagebutton:
            idle "buttons/fishing_catch.png"
            hover "buttons/fishing_catchb.png"
            action SetVariable("fish_caught", "fish4"), Jump("fish_caught")
            pos hook_xpos-50,hook_ypos-50
...
label fish_caught:
    ...
    if fish_caught != "":
        ...
        elif fish_caught == "aqua":
            jump aqua_lure_steal

        if fish_caught != "fish4":
...
image fish4:
    im.Flip("buttons/fishing_fish04a.png", horizontal=True)
    xpos -250
    ypos 700
